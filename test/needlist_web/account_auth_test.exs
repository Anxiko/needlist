defmodule NeedlistWeb.AccountAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, NeedlistWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_account/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      NeedlistWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :redirect_if_account_is_authenticated" do
    ...
    test "doesn't redirect if there is no authenticated account", %{conn: conn} do
      session = conn |> get_session()
